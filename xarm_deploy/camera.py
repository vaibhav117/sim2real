...
class Camera:
	...
	def crop_image_input(self, img, depth):
		if self.view == "side":
			crop_center = [275,325]
			crop_size = [300,300]
			h1 = crop_center[0] - int(crop_size[0]/2) # - 85
			h2 = crop_center[0] + int(crop_size[0]/2)
			w1 = crop_center[1] - int(crop_size[1]/2) # - 85
	...
	def get_frame(self):
		...
		# aligned_depth_frame = self.decimate.process(aligned_depth_frame)
		...
		depth_image = gaussian_filter(depth_image, sigma=2.1)
		depth_image = self.scale_to_255(depth_image)
		depth_image = depth_image/255
		color_image = color_image/255
		
		# return color_image.astype(np.uint8), depth_image.astype(np.uint8) , aligned_color_frame, aligned_depth_frame 
		return np.concatenate((color_image, depth_image[:,:, np.newaxis]), axis=2) , aligned_color_frame, aligned_depth_frame 
